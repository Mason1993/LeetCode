...
public class Solution {
    ...
    public ListNode[] rehashing(ListNode[] hashTable) {
        ...
        ListNode[] table = new ListNode[capacity];
        
        for (int i = 0; i < hashTable.length; i++) {
            ListNode pointer = hashTable[i];
            while (pointer != null) {
                int hashIndex = hashcode(pointer.val, table.length);
                ...
                ListNode insertPlace = table[hashIndex];
    ...
    public int hashcode(int key, int capacity) {
        return (key % capacity + capacity) % capacity;  // key % capacity = (key % capacity + capacity) % capacity in order to handle the situation that key is negative number
    }
};
