    ...
    public int lengthOfLIS(int[] nums) {
        ...
        for (int num: nums) {
            if (list.size() == 0 || list.get(list.size() - 1) < num) {
                ...
                // just curious, list.get(start) > num also works
                if (list.get(0) > num) {   // this is just to handle the case that the first index of list is larger than num. At least if remove the if (list.get(start) > num statement, the test cases {10, 9, 2, 5, 3, 7, 101, 18}; {3, 1, 2}; {10,9,2,5,3,4}; {10,9,2,5,3,7,200,18,101,102,103} will fail the program;
                    list.set(start, num);
